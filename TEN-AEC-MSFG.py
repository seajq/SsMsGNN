...
from model.CAE import CrossAE, MSFEncoder, MSFDecoder, MSFClassifier
...
parser.add_argument('--save', type=str, help='path to save the final model', default='save/TEN/AEC-MSFG')
parser.add_argument('--loss_path', type=str, help='path to save the loss', default='loss/TEN/AEC-MSFG')
...
parser.add_argument('--kernel_set', nargs='+', type=int, default=[9, 6, 3,1])
...
#  python TEN-AEC-MSFG.py --kernel_set 9 6 3 --device cuda:2 --valid_ratio 0.1
#  python TEN-AEC-MSFG.py --kernel_set 9 7 3 --device cuda:2
#  python TEN-AEC-MSFG.py --device cuda:1
#  python TEN-AEC-MSFG.py --unlabeled_ratio 0.1 --device cuda:0
#  python TEN-AEC-MSFG.py --unlabeled_ratio 0.3 --device cuda:1
#  python TEN-AEC-MSFG.py --unlabeled_ratio 0.5 --device cuda:2
#  python TEN-AEC-MSFG.py --unlabeled_ratio 0.7 --device cuda:3
#  python TEN-AEC-MSFG.py --unlabeled_ratio 1 --device cuda:4
# python TEN-AEC-MSFG.py --unlabeled_ratio 0.1;python TEN-AEC-MSFG.py --unlabeled_ratio 0.3;python TEN-AEC-MSFG.py --unlabeled_ratio 0.5;python TEN-AEC-MSFG.py --unlabeled_ratio 0.7;python TEN-AEC-MSFG.py --unlabeled_ratio 1
...
def run(args):
    ...
    model = CrossAE(Encoder=MSFEncoder(gcn_depth=args.gcn_depth, num_nodes=args.num_nodes, device=args.device, kernel_set=args.kernel_set, dropout=args.dropout, subgraph_size=args.subgraph_size,
    ...
    np.save(os.path.join(args.loss_path, f'settings.npy'), vars(args))
...
time = datetime.datetime.now().strftime('%m|%d-%H:%M:%S')
args.loss_path = os.path.join(args.loss_path, time)
