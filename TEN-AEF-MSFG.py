...
parser.add_argument('--save', type=str, help='path to save the final model', default='save/TEN/AEF-MSFG')
parser.add_argument('--loss_path', type=str, help='path to save the loss', default='loss/TEN/AEF-MSFG')
parser.add_argument('--train_ratio', type=float, default=0.7)
parser.add_argument('--test_ratio', type=float, default=0.1)
parser.add_argument('--valid_ratio', type=float, default=0.2)
parser.add_argument('--labeled_ratio', type=float, default=0.01)
parser.add_argument('--unlabeled_ratio', type=float, default=0.3)
...
#  python TEN-AEF-MSFG.py --unlabeled_ratio 0.1 --device cuda:0
#  python TEN-AEF-MSFG.py --unlabeled_ratio 0.3 --device cuda:1
#  python TEN-AEF-MSFG.py --unlabeled_ratio 0.5 --device cuda:2
#  python TEN-AEF-MSFG.py --unlabeled_ratio 0.7 --device cuda:3
#  python TEN-AEF-MSFG.py --unlabeled_ratio 1 --device cuda:4
def run(args):
    ...
    model = FreezeAE(Encoder=MSFEncoder(gcn_depth=args.gcn_depth, num_nodes=args.num_nodes, device=args.device, kernel_set=args.kernel_set, dropout=args.dropout, subgraph_size=args.subgraph_size,
                     ...
                     Decoder=MSFDecoder(gcn_depth=args.gcn_depth, num_nodes=args.num_nodes, device=args.device, kernel_set=args.kernel_set, dropout=args.dropout, subgraph_size=args.subgraph_size,
                                        node_dim=args.node_dim, seq_length=args.seq_length, in_channels=args.in_channels, propalpha=args.propalpha, class_num=args.class_num,
                                        conv_channels=args.conv_channels, end_channels=args.end_channels, out_channels=1),
                     Classifier=MSFClassifier(gcn_depth=args.gcn_depth, num_nodes=args.num_nodes, device=args.device, kernel_set=args.kernel_set, dropout=args.dropout,
